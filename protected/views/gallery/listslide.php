						...
						<?php foreach ($slide as $key => $value) {
						 ...
						 ?>
							<tr>
								...
								<td  class="hidden-xs">
									<img src="<?php echo Yii::app()->request->baseUrl; ?>/images/gallery/<?php  echo $value->image; ?>" width='150'  alt="">
								</td>
								<td class="hidden-xs"><?php echo $value->text; ?></td>
								<td><?php echo $value->datecreate; ?></td>
								<td class="hidden-xs">
									
									<?php echo CHtml::link(" <i class=\"icon-edit\"></i>",array('gallery/editslide', 'id'=>$value['id'],),
									 array(
										 'class'=>'btn btn-green tooltips',
										 'confirm' => 'Are you sure to Edit ?',
									)); ?>

									 <?php echo CHtml::link(" <i class=\"icon-remove icon-white\"></i>",array('gallery/deleteslide', 'id'=>$value['id'],),
									 array(
